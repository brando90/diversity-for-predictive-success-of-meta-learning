...
def meta_train_fixed_iterations(args: Namespace, training: bool = True):
    ...
    # args.it = 0
    # args.meta_learner.train() if args.training else args.meta_learner.eval()
    ...
    while True:
        for batch_idx, batch in enumerate(args.dataloaders['train']):
            ...
            # - scheduler
            if (args.it % 500 == 0 and args.it != 0 and hasattr(args, 'scheduler')) or args.debug:  # call scheduler every
