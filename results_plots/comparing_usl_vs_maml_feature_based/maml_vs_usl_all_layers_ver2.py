#%%
"""
second attempt to join the feature layer plot with the increase then decrease plot difference between uSL vs MAML.
"""
...
x_vals_as_symbols_ = x_vals_as_symbols[:-1]
x_ = x[:-1]
mus3 = list(mus3.values())[:-1]
stds3 = list(stds3.values())[:-1]

# x_vals_as_symbols_ = x_vals_as_symbols
# x_ = x
# mus3 = list(mus3.values())
# stds3 = list(stds3.values())
...
# - d(f_maml, A(f_maml))
plot_with_error_bands(x_vals_as_symbols=x_vals_as_symbols_, x=x_, y=np.asarray(list(mus3)),
...
plot_with_error_bands(x_vals_as_symbols=x_vals_as_symbols, x=x, y=np.asarray(list(mus2)),
                      ...
                      xlabel='Layer', ylabel=f'Distance {metric.upper()}',
                      title=title, curve_label='USL Adapted Head vs Adapted MAML', ylim=ylim)


save_to_desktop(plot_name=f'usl_vs_maml_maml_{metric}2')
plt.show()
# plt.clf()
# plt.close()
...
save_to_desktop(plot_name=f'usl_vs_maml_maml_{metric}_{feature_layers_only=}')
plt.show()
# plt.clf()
# plt.close()
