...
#import wandb
import numpy as np
#run = wandb.init(project="Gradient Descent for Diversity of 1d Gaussian (Hellinger)", entity="patrickyu")
...
def dv(N, M, mu1_b, sigma1_b, mu2_b, sigma2_b):
    #divs = []
    dv_avg = torch.tensor(0.0, requires_grad=True)
    for i in range(N):
        mu_tau1 = mu1_b + sigma1_b * torch.randn(1)#b_dist.sample()#torch.normal(mu_b, sigma_b)
        mu_tau2 = mu1_b + sigma1_b * torch.randn(1) #b_dist.sample()#torch.normal(mu_b, sigma_b)
        sigma_tau1 = torch.abs((mu2_b + sigma2_b * torch.randn(1))) #torch.normal(mu_b, sigma_b)
        sigma_tau2 = torch.abs((mu2_b + sigma2_b * torch.randn(1))) #torch.normal(mu_b, sigma_b)

        #X1 = mu_tau1 + sigma_tau1 * torch.normal(0,1,size=(M,))
        #X2 = mu_tau2 + sigma_tau2 * torch.normal(0,1,size=(M,))
        #print(X1.shape, X2.shape)
        div_coeff = Hellinger(mu_tau1,sigma_tau1,mu_tau2,sigma_tau2)#larger hellinger -> more difference -> more diversity
...
mu1=torch.tensor(mu1_0, requires_grad=True)
sigma1=torch.tensor(sigma1_0, requires_grad=True)
mu2=torch.tensor(mu2_0, requires_grad=True)
sigma2=torch.tensor(sigma2_0, requires_grad=True)
optim = torch.optim.SGD([mu1,sigma1, mu2, sigma2], lr=lr)
...
for i in range(steps):
    diversity = dv(N,M,mu1,sigma1, mu2, sigma2)
    (-diversity).backward() #gradient ascent - we want to increase diversity
    print("new diversity coeff:", diversity)
    print("mu1, sigma1, mu2, sigma2: ", mu1, sigma1, mu2, sigma2)
    print("mu, sigma gradient: ",mu1.grad, sigma1.grad, mu2.grad, sigma2.grad)
    print()
