        ...
        for t in range(nb_tasks):
            ...
            with higher.innerloop_ctx(self.base_model, inner_opt, copy_initial_weights=self.args.copy_initial_weights, track_higher_grads=self.args.track_higher_grads) as (fmodel, diffopt):
                for i_inner in range(self.args.nb_inner_train_steps):
                    ...
                    #inner_train_err = calc_error(mdl=fmodel, X=S_x, Y=S_y)
                    # inner-opt update
                ...
                qrt_loss_t = self.args.criterion(qrt_logits_t,  qry_y_t)
                ## Accumulate gradients wrt meta-params for each task
                qrt_loss_t.backward() # note this is memory efficient
                ...
                qrt_acc_t = calc_accuracy(mdl=fmodel, X=qry_x_t, Y=qry_y_t)
                meta_accs.append( qrt_acc_t )
