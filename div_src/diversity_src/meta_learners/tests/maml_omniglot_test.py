...
from meta_learning.meta_learners.tests.omniglot_loaders import OmniglotNShot
...
def train(db, net, device, meta_opt, epoch, log):
    ...
    print(f'n_train_iter = db.x_train.shape[0] // db.batchsz = {db.x_train.shape[0] // db.batchsz}')
    ...
    for batch_idx in range(n_train_iter):
        ...
        for i in range(task_num):
            ...
            with higher.innerloop_ctx(
                ...
                print(f'y_spt[i].size() = {y_spt[i].size()}')
                print(f'y_spt[i] = {y_spt[i]}')
                print(f'y_qry[i].size() = {y_qry[i].size()}')
                print(f'y_qry[i] = {y_qry[i]}')
                # The final set of adapted parameters will induce some
                # final loss and accuracy on the query dataset.
                # These will be used to update the model's meta-parameters.
                qry_logits = fnet(x_qry[i])
                # print(f'qry_logits.size() = {qry_logits.size()}')
                # print(f'y_qry[i].size() = {y_qry[i].size()}')
                # print(f'qry_logits = {qry_logits}')
                # print(f'y_qry[i] = {y_qry[i]}')
                qry_loss = F.cross_entropy(qry_logits, y_qry[i])
                print(f'---> qry_loss = {qry_loss}')
